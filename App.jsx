...
function PremiumLoader(){return <div className="premiumLoader"><div className="loaderShell"><div className="loaderMark"><i/><b>MD</b></div><h1>MARKET DEVILS</h1><p>Собираю свечи, фракталы, сессии, POI, FTA и новости</p><div className="loadGrid"><span/><span/><span/><span/><span/><span/><span/><span/></div><div className="loadBar"><i/></div><small>EchoDesk · Execution workstation</small></div></div>}
...
function Chart({candles,m}){
  ...
  const levelColor=l=>l.type==='invalid'?'#dc2626':l.type==='rr'?'#059669':l.type==='poi'?'#7c3aed':l.type==='weekly'?'#111827':l.type==='daily'?'#ea580c':l.type==='fractal'?(l.side==='buy'?'#334155':'#64748b'):'#0f766e';
  ...
  return <section className="chart card cleanChart"><div className="chartHead"><div><h2>{m.asset.name}</h2><p>Текущая H1-зона · фрактальная ликвидность · POI / INVALID / TP 1:2+</p></div><div className="quote"><b>{fmt(m.last.close)}</b><span className={m.change>=0?'long':'short'}>{m.change>=0?'+':''}{fmt(m.change,2)}%</span></div></div><svg viewBox={`0 0 ${W} ${H}`}>
    ...
    <line x1={P} x2={W-P} y1={Y(m.last.close)} y2={Y(m.last.close)} stroke="#111827" strokeDasharray="2 6" opacity=".4"/><rect x={W-P-98} y={Y(m.last.close)-14} width="90" height="28" rx="8" fill="#111827"/><text x={W-P-88} y={Y(m.last.close)+4} fill="#fff">NOW {fmt(m.last.close)}</text>
    {m.tda.invalid&&<g><line x1={P} x2={W-P} y1={Y(m.tda.invalid.price)} y2={Y(m.tda.invalid.price)} stroke="#dc2626" strokeDasharray="8 6"/><rect x={P+8} y={Y(m.tda.invalid.price)-25} width="170" height="24" rx="8" fill="#fee2e2"/><text x={P+18} y={Y(m.tda.invalid.price)-9} fill="#b91c1c">INVALID {fmt(m.tda.invalid.price)}</text></g>}
    {m.tda.targets.slice(0,3).map((t,i)=><g key={'targetBadge'+i}><rect x={P+8} y={Y(t.price)-26} width="190" height="24" rx="8" fill="#dcfce7"/><text x={P+18} y={Y(t.price)-10} fill="#047857">{t.name} · RR {t.rr? t.rr.toFixed(1): '2+'}</text></g>)}
  </svg></section>
}
...
function DailyPlan({m}){const rows=scenarioRows(m); const b=m.tda.bias; const poi=m.tda.pois[0]; const inv=m.tda.invalid; const tps=m.tda.targets||[]; const react=poi?`${fmt(Math.min(poi.from,poi.to))}–${fmt(Math.max(poi.from,poi.to))}`:(inv?fmt(inv.price):'после sweep + BOS'); return <section className="dailyPlan card proPanel"><Title t="План на день" s="как читать: реакция → подтверждение → вход → invalid → TP"/><div className="setupBoard"><div className="setupHero"><small>СЕТАП ДНЯ</small><b className={b.toLowerCase()}>{b==='LONG'?'LONG после снятия lows':b==='SHORT'?'SHORT после снятия highs':'WAIT до sweep/BOS'}</b><p>{m.tda.plan}</p></div><div className="setupStep"><small>1 · Откуда ждать реакцию</small><b>{poi?poi.label:(inv?.label||'POI не сформирован')}</b><p>{react}</p></div><div className="setupStep"><small>2 · Что должно подтвердить</small><b>{b==='LONG'?'Sweep sell-side + H1 BOS вверх':b==='SHORT'?'Sweep buy-side + H1 BOS вниз':'Любой sweep + displacement'}</b><p>Без подтверждения это не вход, а только зона наблюдения.</p></div><div className="setupStep danger"><small>3 · Инвалидация</small><b>{inv?`${inv.label} · ${fmt(inv.price)}`:'нет чистого invalid'}</b><p>Если H1 закрывается за invalid без возврата — план отменяется.</p></div><div className="setupStep target"><small>4 · Take Profit</small><b>{tps[0]?`${tps[0].name} · ${fmt(tps[0].price)}`:'TP после BOS'}</b><p>{tps.map(t=>`${t.name}: ${fmt(t.price)} · RR ${Number(t.rr||2).toFixed(1)}`).join(' / ')||'цель не ближе RR 1:2.'}</p></div></div><div className="scenarioCards">{rows.map((r,i)=><article key={i}><h3>{r.title}</h3><dl><dt>Триггер</dt><dd>{r.trigger}</dd><dt>Вход / реакция</dt><dd>{r.entry}</dd><dt>Цели</dt><dd>{r.tp}</dd></dl></article>)}</div><div className="executionRules"><b>Фильтр исполнения</b><span>Не входить в середине диапазона.</span><span>Сначала liquidity sweep, потом displacement.</span><span>Минимальная цель — RR 1:2.</span><span>После новости ждать закрытие свечи, не прыгать в импульс.</span></div></section>}
