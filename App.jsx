...
function Chart({candles,m}){
  ...
  return <section className="chart card cleanChart"><div className="chartHead"><div><h2>{m.asset.name}</h2><p>H1 · сессии · fractal liquidity · POI / FTA / TP уровни</p></div><div className="quote"><b>{fmt(m.last.close)}</b><span className={m.change>=0?'long':'short'}>{m.change>=0?'+':''}{fmt(m.change,2)}%</span></div></div><svg viewBox={`0 0 ${W} ${H}`}>
