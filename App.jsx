...
export default function App(){
  ...
  return <div className="terminal">
    ...
    <main className="grid">
      ...
      <Cockpit m={m} macro={macro} runAI={runAI} loadNews={loadNews}/>
      <DailyPlan m={m}/>
...
function Cockpit({m,macro,runAI,loadNews}){const b=m.tda?.bias||m.bias;return <aside className="cockpit card"><div className="biasLine"><small>BIAS</small><b className={b.toLowerCase()}>{b}</b><span>{m.score}%</span></div><div className="nowPrice"><small>Текущая цена</small><b>{fmt(m.last.close)}</b><em>{m.asset.name} · H1 рабочий график</em></div><div className="tradeMap"><div><small>Зона реакции</small><b>{m.tda.pois[0]?`${fmt(Math.min(m.tda.pois[0].from,m.tda.pois[0].to))}–${fmt(Math.max(m.tda.pois[0].from,m.tda.pois[0].to))}`:'ждать sweep + BOS'}</b></div><div><small>Инвалидация</small><b>{m.tda.invalid?`${m.tda.invalid.label} · ${fmt(m.tda.invalid.price)}`:'нет чистой'}</b></div><div><small>TP 1:2+</small><b>{m.tda.targets[0]?`${m.tda.targets[0].name} · ${fmt(m.tda.targets[0].price)}`:'после подтверждения'}</b></div></div><div className="whatFirst"><small>Что снимаем первым</small><b>{m.first} · {m.firstLevel?.label}</b><div className="probBar"><i style={{width:`${m.firstProb}%`}}/></div><span>{m.firstProb}% · расчёт от фракталов, P/D и session liquidity</span></div><div className="macroMini"><small>Макро фон</small><b className={macro.bias.toLowerCase()}>{macro.bias}</b><p>{macro.text}</p></div><div className="cockpitActions"><button onClick={runAI}>AI-план</button><button onClick={loadNews}>Обновить новости</button></div></aside>}
function MacroPanel({macro,news}){return <section className="macroPanel card"><Title t="Макро-анализ по новостям" s="не придумывает факты · использует загруженные заголовки"/><div className="macroScore"><b className={macro.bias.toLowerCase()}>{macro.bias}</b><span>{macro.text}</span><em>{macro.high} high impact заголовков</em></div><div className="drivers">{macro.drivers.length?macro.drivers.map((d,i)=><p key={i}>• {d}</p>):<p>Новости ещё не загружены или нет Groq-перевода.</p>}</div></section>}
