...
function tdaAnalysis({asset,h1,h4,d1,w1,c5,m}){
  ...
  const lb=lastBreak(h1?.length?h1:c5); const pois=findPOI(h1?.length?h1:c5);
  const prevDay=d1?.at(-2), prevWeek=w1?.at(-2), last=(h1?.at(-1)||c5.at(-1));
  const levels=[];
  if(prevWeek){levels.push({label:'PWH', price:prevWeek.high, type:'weekly', side:'buy'}); levels.push({label:'PWL', price:prevWeek.low, type:'weekly', side:'sell'})}
  if(prevDay){levels.push({label:'PDH', price:prevDay.high, type:'daily', side:'buy'}); levels.push({label:'PDL', price:prevDay.low, type:'daily', side:'sell'})}
  if(m?.sess?.asia?.high) levels.push({label:'Asia High',price:m.sess.asia.high,type:'session',side:'buy'});
  if(m?.sess?.asia?.low) levels.push({label:'Asia Low',price:m.sess.asia.low,type:'session',side:'sell'});
  for(const p of pois) levels.push({label:p.label,price:p.price,type:'poi',side:p.side==='LONG'?'buy':'sell',zone:[p.from,p.to]});
  ...
  const above=levels.filter(l=>l.price>last.close).sort((a,b)=>a.price-b.price); const below=levels.filter(l=>l.price<last.close).sort((a,b)=>b.price-a.price);
  const targets=(bias==='LONG'?above:below).slice(0,3).map((l,i)=>({...l,name:`TP${i+1} · ${l.label}`}));
  const invalid=(bias==='LONG'?below[0]:above[0])||null;
  const fta=targets[0]||null;
  const plan = bias==='LONG'
    ? `Дневной план: работать только от discount/POI после подтверждения на H1. Первый FTA — ${fta?fta.label:'ближайшая buy-side ликвидность'}. Инвалидация ниже ${invalid?invalid.label:'последнего H1 low'}.`
    : bias==='SHORT'
      ? `Дневной план: искать шорт после sweep/breakdown и возврата в premium/POI. Первый FTA — ${fta?fta.label:'ближайшая sell-side ликвидность'}. Инвалидация выше ${invalid?invalid.label:'последнего H1 high'}.`
      : `Дневной план: нет HTF-синхронизации. Ждать sweep одной стороны и H1 displacement перед решением.`;
  const checklist=[`W1: ${W.dir} · ${W.text}`,`D1: ${D.dir} · ${D.text}`,`H4: ${H4.dir} · ${H4.text}`,`H1: ${H1.dir} · ${H1.text}`,`Структура: ${lb.type}${lb.level?' @ '+fmt(lb.level):''}`,`POI рядом: ${pois[0]?.label||'нет чистой зоны'}`]
  return {bias,W,D,H4,H1,lb,pois,levels,targets,invalid,fta,plan,checklist,chart:h1?.length?h1:bucket(c5,60)};
}
...
function Command({m}){const b=m.tda?.bias||m.bias; return <section className="command card"><p>BIAS / план дня</p><h1 className={b.toLowerCase()}>{b==='WAIT'?'WAIT':b}</h1><div className="score"><Ring v={m.score} c={b==='LONG'?'#10b981':b==='SHORT'?'#ef4444':'#eab308'}/><div><b>{m.score}%</b><span>качество сетапа</span></div></div><h3>{m.tda.plan}</h3><div className="targetBox"><small>Цели на H1</small>{m.tda.targets.length?m.tda.targets.map((t,i)=><div key={i}><b>{t.name}</b><span>{fmt(t.price)}</span></div>):<p>Ждём чистую цель после sweep / BOS.</p>}</div><div className="reasonList">{m.tda.checklist.map((r,i)=><span key={i}>✓ {r}</span>)}</div></section>}
function Right({m,runAI,loadNews}){return <aside className="rightcol"><section className="first card"><small>Что вероятнее снимем первым</small><b>{m.first}</b><strong>{m.firstLevel?.label} · {fmt(m.firstLevel?.price)}</strong><Ring v={m.firstProb} c="#0ea5e9"/><p>Расчёт по ближайшей ликвидности, P/D, сессиям и H1 структуре.</p></section><section className="card micro"><h3>Execution</h3><span className={m.displacement?'ok':'bad'}>{m.displacement?'Displacement есть':'Displacement нет'}</span><span className={m.tda.lb.type.includes('BOS')?'ok':m.tda.lb.type.includes('Sweep')?'warn':'bad'}>{m.tda.lb.type}</span><span>{m.smt.state}</span><button onClick={runAI}>Развернутый AI-план</button><button onClick={loadNews}>Новости RU</button></section></aside>}
function Chart({candles,m}){const W=1120,H=560,P=44,hi=Math.max(...candles.map(x=>x.high),...m.tda.levels.map(l=>l.price)),lo=Math.min(...candles.map(x=>x.low),...m.tda.levels.map(l=>l.price)); const X=i=>P+i*(W-2*P)/Math.max(1,candles.length-1),Y=v=>P+(hi-v)*(H-2*P)/(hi-lo||1),step=(W-2*P)/candles.length; const dayStart=dayKey(candles[0]?.time),zones=SESSIONS.map(s=>{let ids=candles.map((c,i)=>({c,i})).filter(o=>hourBE(o.c.time)>=s.start&&hourBE(o.c.time)<s.end&&dayKey(o.c.time)===dayStart); if(!ids.length)return null; return {...s,x:X(ids[0].i),w:X(ids.at(-1).i)-X(ids[0].i)+step}}).filter(Boolean); const levels=[['HOD',m.hi,'#111827'],['LOD',m.lo,'#111827'],['EQ',m.mid,'#94a3b8'],...m.tda.levels.filter(l=>['weekly','daily','poi'].includes(l.type)).slice(0,8).map(l=>[l.label,l.price,l.type==='poi'?'#8b5cf6':l.type==='weekly'?'#0f172a':'#0284c7'])].filter(x=>Number.isFinite(x[1])); return <section className="chart card"><div className="chartHead"><div><h2>{m.asset.name}</h2><p>H1 · Top Down Analysis · цели и POI построены по OHLC</p></div><div className="quote"><b>{fmt(m.last.close)}</b><span className={m.change>=0?'long':'short'}>{m.change>=0?'+':''}{fmt(m.change,2)}%</span></div></div><svg viewBox={`0 0 ${W} ${H}`}>{zones.map(z=><g key={z.id}><rect x={z.x} y={P} width={z.w} height={H-2*P} fill={z.color} opacity=".62"/><text x={z.x+10} y={P+22} fill={z.stroke}>{z.label}</text></g>)}{m.tda.pois.map((g,i)=><rect key={i} x={P} y={Y(Math.max(g.from,g.to))} width={W-2*P} height={Math.max(8,Math.abs(Y(g.from)-Y(g.to)))} fill={g.side==='LONG'?'#10b981':'#ef4444'} opacity=".16"/>)}{levels.map(l=><g key={l[0]}><line x1={P} x2={W-P} y1={Y(l[1])} y2={Y(l[1])} stroke={l[2]} strokeDasharray={l[0]==='EQ'?'4 7':'7 7'} opacity=".72"/><text x={W-P-170} y={Y(l[1])-6} fill={l[2]}>{l[0]} {fmt(l[1])}</text></g>)}{m.tda.targets.map((t,i)=><g key={'tp'+i}><line x1={P} x2={W-P} y1={Y(t.price)} y2={Y(t.price)} stroke="#10b981" strokeWidth="2"/><rect x={W-P-180} y={Y(t.price)-18} width="170" height="24" rx="9" fill="#ecfdf5" stroke="#bbf7d0"/><text x={W-P-170} y={Y(t.price)-2} fill="#047857">{t.name} {fmt(t.price)}</text></g>)}{candles.map((c,i)=>{const up=c.close>=c.open; return <g key={i}><line x1={X(i)} x2={X(i)} y1={Y(c.high)} y2={Y(c.low)} stroke={up?'#374151':'#111827'} opacity=".88"/><rect x={X(i)-step*.32} y={Y(Math.max(c.open,c.close))} width={Math.max(2,step*.64)} height={Math.max(3,Math.abs(Y(c.open)-Y(c.close)))} fill={up?'#ffffff':'#111827'} stroke="#111827" strokeWidth="1"/></g>})}<circle cx={W-P-8} cy={Y(candles.at(-1)?.close||m.last.close)} r="5" fill="#111827"/></svg></section>}
function TDA({m}){return <section className="tda card"><Title t="Top Down Analysis" s="W1 → D1 → H4 → H1"/><div className="tfGrid">{[['W1',m.tda.W],['D1',m.tda.D],['H4',m.tda.H4],['H1',m.tda.H1]].map(([k,v])=><div className={'tf '+v.dir.toLowerCase()} key={k}><small>{k}</small><b>{v.dir}</b><span>{v.text}</span><em>{v.strength}%</em></div>)}</div><div className="planGrid"><div><h3>Где был слом / sweep</h3><p>{m.tda.lb.type}{m.tda.lb.level?` на ${fmt(m.tda.lb.level)}`:''}. Это ключевой триггер для следующего сценария.</p></div><div><h3>POI / FTA</h3><p>POI: {m.tda.pois[0]?.label||'нет чистого FVG'}. FTA: {m.tda.fta?`${m.tda.fta.label} ${fmt(m.tda.fta.price)}`:'после подтверждения'}.</p></div><div><h3>Инвалидация</h3><p>{m.tda.invalid?`${m.tda.invalid.label} · ${fmt(m.tda.invalid.price)}`:'Ждать новый H1 swing.'}</p></div></div></section>}
function SessionBoard({m}){return <section className="sessions card"><Title t="Сессии и тайминг" s="ожидания high/low"/><div className="sessionGrid">{SESSIONS.filter(s=>s.id!=='frank').map(s=>{const e=m.exp[s.id]; return <div className="session" key={s.id}><h3>{s.label}</h3><small>{String(Math.floor(s.start)).padStart(2,'0')}:00-{String(Math.floor(s.end)).padStart(2,'0')}:00</small><div className="matrix"><b></b><b>Short expected</b><b>Long expected</b><span>Low Time</span><em className="red">{e.lowShort}</em><em className="green">{e.lowLong}</em><span>High Time</span><em className="red">{e.highShort}</em><em className="green">{e.highLong}</em></div><div className="miniStats"><b>{Math.round(e.stat.medP)}</b><span>med</span><b>{Math.round(e.stat.avgP)}</b><span>avg</span></div></div>})}</div><Peak rows={m.peaks}/></section>}
...
function RangeDesk({m}){return <section className="stats rangeDesk card"><Title t="Range Desk" s="только самое нужное"/><div className="rangeMain"><div><small>Daily extension</small><b>{Math.round(m.extensionPips)}</b><span>сейчас</span></div><div><small>Средний дневной</small><b>{Math.round(m.stats.daily.avgP)}</b><span>avg</span></div><div><small>Выполнение</small><b>{Math.round(m.dailyCompletion)}%</b><span>от avg</span></div></div><div className="cleanStats"><Stat title="Asia range" data={m.stats.asia}/><Stat title="IB size" data={m.stats.ib}/><Stat title="IB extension" data={m.stats.ibExt}/></div><div className="pd"><span>Discount</span><div><i style={{left:`${m.pd}%`}}/></div><span>Premium</span></div></section>}
